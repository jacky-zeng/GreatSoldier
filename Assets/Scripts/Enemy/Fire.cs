...
public class Fire : BaseEnemy
{
    ...
    void Start()
    {
        ...
        ////因为Fire这个敌人更大，需要改变上挑吸附距离
        //xiFuLeft = new Vector3(3.5f, 2.5f, 0);
        //xiFuRight = new Vector3(-3.5f, 2.5f, 0);
        enemyAvatarSprite = Resources.Load<Sprite>("Images/Enemy/Fire/enemyAvatar") as Sprite;
    }
    #endregion
        ...
        if (isBeginReal)
        {
            ...
            else
            {
                ...
                else if (!isHitOnGround && !isJumpHit)
                {
                    ...
                    if (!isAttackHeavy && (Mathf.Abs(distance.x) >= 2.7f || Mathf.Abs(distance.y) > 1.5f || Mathf.Abs(distance.z) > 2f))
                    {
    ...
    public void begin(bool isAttackRightNow = false)
    {
        if (!isBegin)
        {
            isBegin = true;
    ...
    private void attackHeavy()
    {
        ...
        {
            playAudio("Audios/Enemy/Fire/Fire");
            //使攻击的朝向和主体保持一致
